...
- `docs/README.md` 面向模板仓库维护者，作为仓库级开发维护文档、机制参考和仓库经验的导航，不复制具体 Mod、共享项目或
  模板的可变清单。
- `docs/development/README.md` 面向模板维护者，说明外部依据、维护检查、工具安装、生成项目与打包验证、发布流水线维护和结构入口。
...
- 共享项目和源码子目录的 README 说明各自的模块边界、API、特殊运行边界和项目内约定。
...
源码迭代说明留在对应
`mods/<ModName>/DEVELOPMENT.md`、`mods/<ModName>/docs/` 或源码子目录 README；共享库 API、额外运行时依赖和特殊端侧要求留在
...
新增、移除或重命名内部共享项目时，同步更新项目自己的 README、项目文件、lock file、解决方案注册和引用方项目文件中的
`ProjectReference`。如果仓库在 `shared/README.md` 维护一级目录索引，再同步更新索引。

现有 Mod 的真实组包以项目文件、`Taiwu.Mod.Pack.proj`、lock file、目录 README 和解决方案注册为准；现有共享项目的目标框架、
引用和公开 API 约定以项目文件、项目 README、lock file、引用方 `ProjectReference` 和解决方案注册为准。`templates/` 只用于创建新项目的初始骨架。
...
模板只描述新项目的初始骨架。现有 Mod 的真实构建和组包约定以项目文件、`Taiwu.Mod.Pack.proj`、目录 README、lock file 和
解决方案注册为准；现有共享项目的目标框架、引用和公开 API 约定以项目文件、项目 README、lock file 和引用方
`ProjectReference` 为准。

模板会生成新项目文档和 `Config.Lua` 展示字段占位。调整文档关系、最小入口文案、组包入口说明或共享项目边界时，同步复核
[`templates/README.md`](../../templates/README.md) 中列出的输出文案模板，让新项目沿用当前文档关系。

面向 Mod 使用者的 README 模板保持克制，只说明 Mod 身份并指向源码维护入口；具体功能、安装方式、品牌表达、发布文案和信息结构
由具体 Mod 维护。面向维护者的模板说明构建和组包边界；模板仓库维护细节归本仓库文档。
